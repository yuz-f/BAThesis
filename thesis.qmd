...
The reported/actual truthfulness distinction in the model has the formal structure of a signaling problem [@Spence1973]: authors possess private knowledge of their model's true quality while the community observes only the inflated signal. The competitive pressure scaling — which raises bias inflation for researchers whose reputation falls below the field median — captures the incentive structure of cheap signaling under career pressure, where low-status senders have the most to gain from signal inflation [@Akerlof1970]. The epistemic landscape sharpens this asymmetry spatially: models published from peak positions receive the greatest additional inflation, precisely where replication is hardest and verification most costly — a structural analogue to adverse selection concentrating in the most attractive market segments. The model does not implement a full strategic signaling equilibrium; inflation is drawn stochastically rather than chosen optimally by a rational sender, and there is no upfront cost that would enforce honesty in the manner of Zahavi's handicap principle. What it captures is the pre-equilibrium pressure that makes the scientific signaling environment systematically unreliable, independent of individual bad faith.
