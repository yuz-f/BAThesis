...
def _worker(task):
    ...
    scenario = PEAKED if scenario_name == 'PEAKED' else BROAD
...
if __name__ == "__main__":
    ...
    for tt, sga in itertools.product(TT, SGA):
        for seed in SEEDS:
            tasks.append(('PEAKED', tt, sga, seed))
            tasks.append(('BROAD',  tt, sga, seed))
    ...
    for tt, sga in itertools.product(TT, SGA):
        sf = float(np.mean(agg[('PEAKED', tt, sga)]))
        gf = float(np.mean(agg[('BROAD',  tt, sga)]))
        gap = sf - gf
