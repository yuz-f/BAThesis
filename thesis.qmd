...
**Action selection.** At the start of each step (if the researcher is not mid-exploitation), available models are partitioned into *exploit candidates* (complexity gap ≤ training threshold) and *invest candidates* (gap > threshold). The top two models by expected value are retained, and an explore option is added. The researcher then selects probabilistically among these options. The selection weight for each candidate is scaled by a *skill bias* term,
