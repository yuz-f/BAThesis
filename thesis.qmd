...
\begin{abstract}
\noindent
The replication crisis in psychology has been attributed to incentive structures and statistical norms, but the role of researcher skill distribution has not been formally modelled. This thesis presents an agent-based computational model of a stylised scientific community in which fifty researchers, distributed across ten laboratories and ten research domains, produce and evaluate scientific models over 300 discrete time steps under evolutionary selection. Two scenarios are compared: a Specialist community, in which researchers have one pronounced domain expertise and weak secondary skills, and a Generalist community, in which researchers maintain a meaningful baseline across all domains alongside their primary expertise. Across 30 independent simulation runs per scenario, the specialist community produced a significantly higher replication failure rate (.62 vs.\ .59; Mann-Whitney \textit{U} = 638.0, \textit{p} = .006, \textit{d} = 0.78) and greater concentration of publications in fewer domains (Gini .139 vs.\ .093; \textit{p} < .0001, \textit{d} = 2.14). Researcher reputation did not differ significantly between scenarios (\textit{p} = .684), but its composition differed structurally: specialists achieved equivalent career outcomes through fewer, higher-yield publications in their peak domain, while generalists produced more publications at lower per-event return. An exploratory hypothesis that specialists would be more vulnerable to reputational debunking was directionally reversed — generalist publications showed higher debunking susceptibility — but this difference did not reach statistical significance (\textit{p} = .165). A third \textit{Uniform} control scenario — flat skill profiles, no preferred domain — substantially sharpened the headline interpretation: the concentration finding (H2) was confirmed as a clean function of skill heterogeneity (Gini under Uniform skills: .075, lower than both other scenarios; \textit{d} > 0.8 in both control comparisons), while the failure-rate finding (H1) was revealed to be U-shaped rather than monotonic, with Uniform skills producing the highest failure rate of all three (.645). Three methodological analyses sharpened the interpretation. (a) Leave-one-out cross-validated linear regression recovered the manipulated parameter from outcome metrics with $R^2 = 0.96$, supporting the claim that observed differences track the manipulation rather than seed noise. (b) The model produces per-researcher publication inequality at a Gini of 0.41 — qualitatively consistent with documented academic productivity skew but below the empirical 0.50–0.70 band, a quantitative boundary explained by the model's selection-cull mechanism and finite simulation horizon. (c) A layer-decomposition ablation showed that the H1 effect is essentially absent ($d = 0.04$) when the model is run with only base dynamics and emerges only when realism extensions interact with skill heterogeneity, refuting the strongest version of the tautology critique that specialists must fail more "by definition" of the success-probability rule; the H2 effect, by contrast, has a substantial structural component visible at all layers. The directional results follow from how skill enters action selection and replication probability in the model and are therefore best read as quantifications of mechanism magnitude rather than tests of skill distribution as a causal variable in real science. Together, the simulations characterise the dynamics, scale, and parameter sensitivity of replication-rate and concentration differences that emerge from a particular formalisation of skill heterogeneity, and suggest that the communicative infrastructure of a discipline — how broadly researchers can evaluate work outside their primary expertise — is a tractable lever for formal modelling alongside incentive structures.
...
The simulation model consists of three entity types — Labs, Researchers, and Scientific Models — interacting across ten research domains. Figure \ref{fig:model-diagram} provides a schematic overview of the model architecture and the causal relationships between its components.
...
Figure \ref{fig:landscape} shows one domain's epistemic landscape together with the spatial distribution of models that the two scenarios produce by step 300. Panels A and B render the height function $h(x, y)$ as a 3D surface and a top-down heatmap, respectively, with the seed model marked. Panels C and D show, for the same domain, the position of every model published over 300 steps under each scenario, with currently-active models highlighted. The visual contrast between Panels C and D previews the quantitative results that follow: specialist research programmes produce active literatures concentrated in the valley region (left), while generalist scenarios produce a more dispersed active literature. The publication-history background (faint dots) shows that both scenarios *publish* across the full theory-space, but only valley-positioned work *survives* as the actively cited literature.
...
## Scenarios: Specialist vs. Generalist {#sec-scenarios}
...
: Fixed and varied simulation parameters across the three scenarios. The Uniform scenario sets peak and non-peak parameters equal so that all researchers have flat skill profiles centred near 0.40, with no preferred domain. {#tbl-params}
...
The structural mechanism underlying this directional pattern is informative even in the absence of significance. Specialist researchers publish primarily in their peak domain, where their high skill produces models with above-average actual truthfulness. Equation 5 shows that debunk success probability scales with $(1 - \theta_{\text{actual}})$ and is further amplified for models in unstable theory-space positions: specialist research programmes, which converge toward valley positions via gradient descent, benefit from both high truthfulness and high landscape stability, making their output doubly resistant to challenge. Generalist researchers publish across multiple domains at intermediate skill levels, producing models with lower average truthfulness that are exposed at less stable landscape positions. The introduction of the epistemic landscape appears to have increased variance in the debunking process — the instability boost raises debunking rates generally, compressing the proportional difference between scenarios and widening confidence intervals — which accounts for the loss of significance relative to the earlier model. The direction of the effect, however, is robust across both model versions.
...
The action-distribution data confirm that the model's internal mechanics are behaving as expected. Across all steps pooled within each scenario, exploit steps constitute 19.5% of activity for specialists and 33.2% for generalists; training steps constitute 56.4% and 43.1% respectively; explore steps are nearly equal (24.1% vs. 23.6%). The elevated training burden for specialists directly reflects the model's training threshold mechanism: specialists frequently encounter models whose complexity in secondary domains exceeds their skill, triggering sustained training investment that does not translate into publications. This time cost is the proximal cause of the lower exploitation rate, which in turn limits the diversity of models that specialist researchers engage with and replicate. The higher explore fraction compared to earlier model iterations (≈24% vs. ≈19%) reflects the epistemic landscape's effect on exploration value: novel theory-space positions are more attractive when the gradient toward valleys draws researchers toward productive territory.
...
## Control Check: Uniform Skill Distribution {#sec-control}
...
\captionsetup{labelformat=apa, labelsep=newline, justification=raggedright}
\caption{\textit{Layer-decomposition ablation: SPEC vs GEN means and effect sizes for H1 and H2 across three nested model layers.}
...
H4 — debunking stability — showed a directional reversal that did not reach significance (*p* = .165, *d* = −0.40). Specialist researchers lost less reputation to debunking than generalists, consistent with the direction found in the earlier model iteration (*d* = −0.77, *p* = .0008), but the effect is attenuated in the current version. The structural logic of the reversal remains intact: Equation 5 makes debunk probability dependent on proficiency, actual untruthfulness, and the instability boost from the epistemic landscape. A specialist's published models in their peak domain are built on genuine high-skill work and tend toward stable valley positions through gradient-descent convergence; they are therefore more truthful on average and occupy less vulnerable landscape positions, making them doubly hard to debunk. Generalist models, published across domains at intermediate skill levels and from less settled landscape positions, are more exposed. The attenuation of the effect relative to the earlier model reflects the introduction of the instability boost: by raising debunking rates generally — particularly for peak-positioned models, which appear in both scenarios — the landscape compresses the proportional difference between specialist and generalist vulnerability. The direction of the effect is robust; its magnitude is sensitive to the epistemic landscape dynamics.
...
<!-- VOICE OWNERSHIP: Added in commit c90aa96 / refined in d6c875e (tier 3.3). Technical economic-theory register that may not match the rest of your thesis. Read for tonal consistency with your Introduction; rewrite if it reads as bolted-on. -->


The reported-versus-actual truthfulness distinction has the formal structure of an information-asymmetry problem [@Spence1973; @Akerlof1970]: authors hold private knowledge of their model's true quality, while the community sees only the inflated signal. The competitive-pressure scaling — under which researchers with below-median reputation inflate more — gives the bias term a partial signaling-theoretic character: low-status senders have the most to gain from raising the signal, exactly the dynamic that produces adverse selection in classical lemons markets. The landscape adds a spatial layer to this. Peak positions accrue the greatest additional inflation while also making replication hardest, so models in apparently exciting (high-reported-truth) territory are precisely where the gap between reported and actual is largest — the model's analogue of adverse selection concentrating in segments where buyers find it hardest to verify quality.

The analogy has limits that should be named. Classical signaling equilibria require either costly signals that only high-quality senders can afford [@Spence1973] or buyers who withdraw from markets they cannot evaluate; the present model implements neither. Inflation is drawn stochastically rather than chosen optimally, there is no upfront cost that would enforce honesty in the manner of Zahavi's handicap principle, and the simulated community has no mechanism for collective withdrawal from a domain when its credibility falls. What the model captures is therefore not a full strategic signaling equilibrium but the *pre-equilibrium pressure* that produces systematically unreliable signals: the asymmetry exists, the incentive to inflate exists, and the structural conditions that would discipline either are not yet present. Reading the results through this lens helps explain why publication bias persists even with no individual bad faith, and why peak theoretical territory is most prone to inflated reporting; reading it as evidence about strategic behaviour by rational researchers would overshoot what the design supports.
